...
from image_ops_impl import ssim
	...
	with tf.Session(config=config) as sess:
		...
		for epoch in xrange(0, MAX_EPOCH):
			for step in range(len(train_list)//BATCH_SIZE):
				...
				if not saved:
					saver.save(sess, "/usr/project/xtmp/yb44/EDSR_9_checkpoints/initial.ckpt")
					saved = True
				if epoch+(float(step)*BATCH_SIZE/len(train_list))/0.5 > counter:
					saver.save(sess, "/usr/project/xtmp/yb44/EDSR_9_checkpoints/EDSR_const_clip_0.01_epoch_%03d.ckpt" % epoch ,global_step=global_step)
					counter += 1
